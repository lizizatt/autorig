...
#define OUT { vertices.clear(); edges.clear(); return; }
    ...
    if(file.length() < 4) {
        cout << "I don't know what kind of file it is" << endl;
        return;
    }
    ...
    for(i = 0; i < (int)edges.size(); ++i) { //make sure all vertex indices are valid
        if(edges[i].vertex < 0 || edges[i].vertex >= verts) {
            cout << "Error: invalid vertex index " << edges[i].vertex << endl;
            OUT;
        }
    }
    ...
    if(integrityCheck())
        cout << "Successfully read " << file << ": " << vertices.size() << " vertices, " << edges.size() << " edges" << endl;
    else
        cout << "Somehow read " << file << ": " << vertices.size() << " vertices, " << edges.size() << " edges" << endl;
    ...
    while(!strm.eof()) {
        ...
        if(!outOfHeader) { //look for end_header or verts
            if(words[0] == string("end_header")) {
                if(vertsLeft < 0) {
                    cout << "Error: no vertex count in header" << endl;
                    OUT;
                }
                outOfHeader = true;
                continue;
...
void Mesh::readGts(istream &strm)
{
    ...
    while(!strm.eof()) {
        ...
        if(edgesLeft > 0) {
            --edgesLeft;
            if(words.size() != 2) {
                cout << "Error (edge) on line " << lineNum << endl;
                OUT;
            }
            int e1, e2;
    ...
    while(!strm.eof()) {
        ...
        if(words[0] == string("endfacet")) {
            if(lastIdxs[0] == lastIdxs[1] || lastIdxs[1] == lastIdxs[2] || lastIdxs[0] == lastIdxs[2]) {
                cout << "Duplicate vertex in triangle" << endl;
                continue;
            }
            int first = edges.size();
            edges.resize(edges.size() + 3);
...
#define CHECK(pred) { if(!(pred)) { cout << "Mesh integrity error: " #pred << " in line " << __LINE__ << endl; return false; } }
