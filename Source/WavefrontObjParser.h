...
*/
class WavefrontObjFileWithVertexColors
{
public:
    WavefrontObjFileWithVertexColors() {}

    Result load (const String& objFileContent, const String& mtlFileContent)
    {
        shapes.clear();
        return parseObjFile(StringArray::fromLines (objFileContent), StringArray::fromLines (mtlFileContent));
    }
    
    Result load (const String& objFileContent)
    {
        shapes.clear();
        return parseObjFile (StringArray::fromLines (objFileContent), StringArray());
    }
...
private:
    ...
    struct IndexMap
    {
        ...
        Index getIndexFor (TripleIndex i, Mesh& newMesh, const Mesh& srcMesh)
        {
            ...
            //<<<JAUNT
            if (isPositiveAndBelow (i.vertexIndex, srcMesh.colors.size()))
                newMesh.colors.add (srcMesh.colors.getReference (i.vertexIndex));
            //>>>JAUNT
    ...
    static Color parseColor (String::CharPointerType t)
    {
        Color c;
        parseFloat (t);
        parseFloat (t);
    ...
    Result parseObjFile (const StringArray& lines, const StringArray& mtlLines)
    {
        Mesh mesh;
        ...
        for (int lineNum = 0; lineNum < lines.size(); ++lineNum)
        {
            ...
            {
                Result r = parseMaterial (knownMaterials, String (l).trim(), mtlLines);
                continue;
            }
    ...
    Result parseMaterial (Array<Material>& materials, const String& filename, const StringArray& mtlLines)
    {
        StringArray lines;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WavefrontObjFileWithVertexColors)
};
